...
\author{Pierre-Léo Bourbonnais, Yannick Brosseau, Catherine Morency \\
...
explicitly flagged the centroid-vs-entrance comparison as a 
...
tessellates the world into $10\times10$\,km cells over population
density using GHS-POP and built volume using GHS-BUILT-V
(Joint Research Centre, European Commission) and draws cells with
...
the map on a randomly selected POI, lets the analyst draw, classify and
...
Across the 25 paired observations, 24 have longer walk from
centroid to nearest road network than from entrance (single exception where the centroid is
closer than the entrance). The pooled difference is $+$16\,m median,
